...
class LoadData : ViewModel() {
    ...
    fun loadExampleTitles(){
        Log.i("XPND", "load Example Titles Called")
        CoroutineScope(Dispatchers.IO).launch{
            val databaseReference : DatabaseReference = FirebaseDatabase.getInstance().getReference("E_TITLES")
            databaseReference.addValueEventListener(object: ValueEventListener {
                override fun onDataChange(snapshot: DataSnapshot) {
                    if(snapshot.exists()){
                        Log.i("XPND", "Chapter titles exist")
                        ObjectsCollection.exampleTitlesList.clear()
                        for(title in snapshot.children){
                            val currTitle = title.getValue(Title::class.java)
                            ObjectsCollection.exampleTitlesList.add(currTitle!!)
                        }
                        areExampleTitlesLoaded.value = true
                        ...
                        Log.i("XPND", "Example titles do not exist")
                    }
                }
                override fun onCancelled(error: DatabaseError) {
                    Log.i("XPND", error.toString())
    ...
    fun loadCurrExample(){
        CoroutineScope(Dispatchers.IO).launch {
            FirebaseDatabase.getInstance().getReference("USERS").child(Firebase.auth.currentUser!!.uid).child("CURR_EXAM").get().addOnSuccessListener {
                Log.i("EDP", it.toString())
